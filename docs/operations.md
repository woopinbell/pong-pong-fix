...
Apple M1 8코어·메모리 16GB 장비에서 Docker에 CPU 8개와 약 7.65GiB를 배정해 연결 500개·방 50개 조건을 두 번 실행했습니다.

| 항목 | 1차 실행 | 2차 실행 | 기준 |
| --- | ---: | ---: | ---: |
| 최초 연결 | 500/500 | 500/500 | 99% 이상 |
| 경기 중 재접속 | 96/100 | 95/100 | 99% 이상 |
| 최대 활성 방 | 50 | 50 | 50개 이상 |
| 스냅샷 지연 p95 | 7ms | 9ms | 150ms 이하 |
| 스냅샷 지연 p99 | 20ms | 22ms | 250ms 이하 |
| 이벤트 루프 지연 p95 | 26.066943ms | 26.181631ms | 50ms 이하 |
| 정상 구간 순번 누락 | 0/10,061 | 0/11,214 | 1% 미만 |
| 결과 확정 | 성공 50, 실패·중복 0 | 성공 50, 실패·중복 0 | 실패·중복 0 |

두 실행 모두 재접속 성공률만 기준에 미치지 못했습니다. 따라서 이 조건을 단일 인스턴스 통과 결과로 기록하지 않습니다. 실패가 확인된 위치는 재접속 흐름이며, 분산 세션이나 방 소유권의 한계는 확인되지 않았으므로 Redis와 다중 인스턴스도 추가하지 않습니다. 다음 측정에서는 실패한 연결의 티켓 발급, 기존 연결 교체와 15초 방 보존 단계를 따로 기록해야 합니다.

같은 최종 코드에 DB 응답 300ms 지연과 중단·복구, 공개 경로 150ms 지연과 연결 재설정을 차례로 넣었습니다. DB 중단 시 준비 상태는 503으로 내려갔고 프록시 복구 뒤 200으로 돌아왔습니다. 공개 경로 재설정은 네트워크 오류로 관찰됐으며 경로를 다시 연 뒤 준비 상태가 회복됐습니다. API 컨테이너는 재시작 없이 실행 상태를 유지했고 로그에는 오류 종류와 이름, 코드만 남았습니다.

원본 수치는 [부하 측정 결과](./measurements/load-2026-07-24.json)와 [복구 측정 결과](./measurements/fault-recovery-2026-07-24.json)에 보관합니다. 이번에는 연결 1,000개 선택 조건을 다시 실행하지 않았습니다.
