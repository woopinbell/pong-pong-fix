...
2026-07-24에 Apple M1 장비에서 연결 500개와 경기방 50개 조건을 두 번 실행했습니다. 최초 연결, 방 수, 스냅샷 지연과 경기 결과 확정 기준은 두 번 모두 충족했지만 재접속은 각각 96/100, 95/100으로 99% 기준을 넘지 못했습니다. 전체 통과 결과로 해석하지 않으며, 재접속 경로를 다음 확인 대상으로 남겼습니다.

같은 코드에서 PostgreSQL 지연·중단과 공개 경로 지연·연결 재설정을 주입한 복구 시나리오는 모두 통과했습니다. 세부 조건과 원본 수치는 [부하 측정 결과](docs/measurements/load-2026-07-24.json)와 [복구 측정 결과](docs/measurements/fault-recovery-2026-07-24.json)에 있습니다.

## 현재 범위

- 개발 로그인은 `development`와 `test` 모드에서만 사용할 수 있습니다.
- OAuth 공급자와 외부 호스팅 설정은 포함하지 않습니다.
- 운영 비밀 값은 저장소에 두지 않습니다.
- 비회원 경기 결과는 일반 사용자 기록과 순위표에 반영하지 않습니다.
